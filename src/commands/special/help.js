...
export default {
  ...
  code: `
    ...
    $arrayLoad[bracketsInfo;,;[\\] - required argument(s),| - 'OR' argument,() - optional argument(s) or option,<> - data input]
      ...
      $addContainer[
        ...
        $arrayForEach[categories;category;
          ...
          $arrayForEach[allCommandsInfo;obj;
            $if[$env[category]==$env[obj;commandCategory];
              $arrayPush[ctg;$env[obj;commandAliases;0]]
            ]
          ]
...
function replacements() {
  return `[
    ...
    ["{PUMPKINS_RARITIES}", "\`\`\`JSON(N)$arrayJoin[pumpkinsRarities;(N)]\`\`\`"\\],
    ["{BEACHBALLS_RARITIES}", "\`\`\`JSON(N)$arrayJoin[beachballsRarities;(N)]\`\`\`"\\],
    ["{UMBRELLAS_RARITIES}", "\`\`\`JSON(N)$arrayJoin[umbrellasRarities;(N)]\`\`\`"\\],
    ["{HYPERLINK_TIMEZONES}", "$get[timezonesHyperlink]"\\],
    ["{EMOJI}", "$get[emoji]"\\],
    ["{MAX_COINFLIP_BET}", "$separateNumber[$get[maxCoinflipBet];,]"\\],
    ["{MAX_SLOTS_BET}", "$separateNumber[$get[maxSlotsBet];,]"\\]
  \\]`
}